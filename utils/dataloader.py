...
def _worker_fn(samples, batchify_fn, current_it, dataset=None):
    """Function for processing data in worker process."""
    # pylint: disable=unused-argument
    # it is required that each worker process has to fork a new MXIndexedRecordIO handle
    # preserving dataset as global variable can save tons of overhead and is safe in new process
    _worker_dataset._current_it = current_it
...
class _MultiWorkerIter(object):
    ...
    def _push_next(self):
        ...
        if r is None:
            return
        # print(self._data_loader._current_it)
        async_ret = self._worker_pool.apply_async(
            self._worker_fn, (r, self._batchify_fn, self._data_loader._current_it, self._dataset))
...
class DataLoader(object):
    ...
    def __call__(self, current_epoch, **kwargs):
